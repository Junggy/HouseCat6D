...
class HouseCat6DTrainingDataset(Dataset):
    ...
    def _read_data(self, image_index):
        ...
        mask = cv2.imread(img_path.replace('rgb','instance'))[:, :, 2] # TODO 1096x852
        ...
        dop = dop[rmin:rmax, cmin:cmax].astype(np.float32)
        aop = np.arctan2(p2 - p4, p1 - p3)
        aop += (aop < 0) * np.pi
        aop = np.linalg.norm(aop, axis=-1)
        aop = aop[rmin:rmax, cmin:cmax].astype(np.float32)
        # cv2.imshow('dop', dop)
        # cv2.waitKey(0)
        # cv2.imshow('aop', aop)
        # cv2.waitKey(0)
