...
class Net(nn.Module):
    def __init__(self, resolution=64, ds_rate=2):
        ...
        self.spherical_fpn = SphericalFPN(ds_rate=self.ds_rate, dim_in2=5)
        self.v_branch = V_Branch(resolution=self.ds_res)
        self.i_branch = I_Branch(resolution=self.ds_res)

    def forward(self, inputs):
        rgb_pol = inputs['rgb_pol']
        pts = inputs['pts']
        dis_map, rgb_pol_map = self.feat2smap(pts, rgb_pol)
        
        # backbone
        x = self.spherical_fpn(dis_map, rgb_pol_map)
